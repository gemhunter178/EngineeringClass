...
int main(){
	...
	cout<< "\nThe grid size is " << Elevations.size() << " by " << Elevations[1].size() << endl;
	//bug fixing
...
int findPeak(vector<float> &peaks, vector<vector<float>> &Elevations){
	...
	while(rowNum != Elevations.size() - 1){
		...
		Elevations[rowNum][loopNum] > Elevations[rowNum+1][loopNum]){
			...
			cout<< "Elevation " << peaks[peakCount*3] << " at point " << peaks[peakCount*3+1] + 1 << ", " << peaks[peakCount*3+2] + 1 << endl;
			if(peaks[peakCount*3] > bigPeak){
				bigPeak = peaks[peakCount*3];
				bigPeakRow = peaks[peakCount*3+1] + 1;
				bigPeakCol = peaks[peakCount*3+2] + 1;
			}
	...
	if(peakCount != 0){
		cout<< "\nThe highest peak is at an elevation of " << bigPeak << " at " << bigPeakRow << ", " << bigPeakCol << " in the grid.matrix" << endl;
	} else {
		cout<< "\n...there were no peaks";
	}
	return peakCount;
